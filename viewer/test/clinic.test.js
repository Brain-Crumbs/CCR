...
function fixture() {
  ...
  fs.writeFileSync(path.join(dir, "episode_00000.summary.json"), JSON.stringify({ duration_ticks: 1, success: false, termination_reason: "fixture failure", program_stats: { pixel_sources: ["grid"] } }));
...
test("service lists by organism and returns streams, exports, and verdict", async (t) => {
  ...
  assert.match(listed.body.sessions[0].quality.issues[0], /terminated early/);
...
test("React browser renders grouped session and quality badge from fixture", async () => {
  const source = fs.readFileSync(path.join(__dirname, "../public/session-browser.js"), "utf8");
  const { createSessionBrowser } = await import(`data:text/javascript;base64,${Buffer.from(source).toString("base64")}`);
  ...
  const Browser = createSessionBrowser(React);
  const tree = Browser({ initialSessions: [{ id: "pixel-session", name: "Pixel", episodes: ["episode_00000"], quality: { verdict: "amber", issues: [], warnings: ["motion check close to floor"] } }] });
  function text(node) { if (node == null) return ""; if (typeof node !== "object") return String(node); if (typeof node.type === "function") return text(node.type(node.props)); return node.children.map(text).join(" "); }
  assert.match(text(tree), /Pixel/); assert.match(text(tree), /pixel-session/); assert.match(text(tree), /amber/); assert.match(text(tree), /motion check close to floor/);
});
