...
def test_collision_only_suffixes_the_new_name_and_keeps_run_identity_separate():
    ...
    second = assign_display_name(
        run_id="second-deadbeef", naming_seed="same-seed", existing_names=(base.display_name,),
    )
    assert second.display_name == f"{base.display_name}-deadbeef"
    assert base.display_name != second.display_name
    child = assign_display_name(
        run_id="child", naming_seed="child", parents=(DisplayNameParent("second-deadbeef", second.display_name),),
    )
