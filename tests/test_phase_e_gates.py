"""Phase E evaluation gate 1 (issue #31): the actor/critic beats random on
identical seeds.
...
(the CLI ``phase-e-gates`` subcommand, docs/online-learning.md), so a shift in
...
from cognitive_runtime.training.phase_e_gates import (  # noqa: E402
    DEFAULT_GATE_CONFIG,
    GATE_METRIC,
    PhaseEGateResult,
    run_phase_e_gates,
...
def test_phase_e_gate1_beats_random_with_recordings_and_checkpoint(tmp_path):
    ...
    assert (record_dir / "phase-e-actor-critic").is_dir()
    assert (record_dir / "phase-e-online-q").is_dir()
    assert (record_dir / "phase-e-scripted").is_dir()
    assert (record_dir / "phase-e-random").is_dir()

    # Gate results written into the checkpoint bundle's training stats (#20).
    meta = read_checkpoint_metadata(str(checkpoint))
    gate_report = meta["training_stats"]["phase_e_gates"]
